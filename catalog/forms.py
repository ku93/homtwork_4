...
class ProductForm(StyledFormMixin, ModelForm):
    ...
    def clean_image(self):
        ...
        if (
            ...
            and image.content_type not in VALID_IMAGE_FORMATS
        ):
            raise forms.ValidationError(
                f'Допустимые форматы изображений: {", ".join(VALID_IMAGE_FORMATS)}.'
            )

        if image.size > MAX_IMAGE_SIZE:
            raise forms.ValidationError(
                f"Максимальный размер файла - {MAX_IMAGE_SIZE / 1024 / 1024:.2f} МБ."
            )
